<template>
  <div>
    ...
    <div class="demo-content">
      ...
      <date-picker :searchFormDate.sync="date"
                  type="datetimerange"
                  :isLimit='false'
                  dateType="yyyy-MM-dd HH:mm:ss"
                  @dateChange="dateChange"></date-picker>
      <p>当前选择时间: {{date}}</p>
      
      <p class="demo-title">弹窗组件</p>
      <div>
        <span class="button-span"
          @click="popFormDialog">打开confirm形式弹窗</span>
        <span class="button-span"
          @click="popInformDialog">打开inform形式弹窗</span>
      </div>
      <pop-dialog dialogType="form"
                  title="弹窗标题"
                  :showFlag.sync="showFormDialogFlag"
                  centerStyle
                  showCloseFlag
                  :width="500"
                  cancelbuttonText="取消"
                  buttonText="确定"
                  :beforeCloseFun="handleBeforeCloseDialog"
                  @sureClick="handleSure()"
                  @cancelClick="showFormDialogFlag = false">
        <p>自定义内容</p>
      </pop-dialog>
      <pop-dialog dialogType="inform"
                  title="弹窗标题"
                  :height="100"
                  clickBgCloseFlag
                  @close="handleCloseDialog"
                  :showFlag.sync="showInformDialogFlag"
                  @sureClick="showInformDialogFlag = false">
        <p>自定义内容</p>
      </pop-dialog>

      <p class="demo-title">多选选择器组件</p>
      <span class="label-span">多选普通: </span>
      <multi-select :type="'game'"
                    :options="optionsList"
                    :placeholder="'游戏'"
                    :parentVal="selectValue"
                    @update="updateSelect"></multi-select>
      <span class="label-span">多选分组: </span>
      <multi-select :type="'channel'"
                    :options="optionsGroup"
                    :placeholder="'渠道'"
                    :group="true"
                    :parentVal="selectValueGroup"
                    @update="updateSelect"></multi-select>
      
      <p class="demo-title">自定义筛选组件</p>
      <span class="button-span"
            @click="showFilter">打开自定义筛选弹窗</span>
      <customize-filter title="自定义列"
                        sureButtonText="应用"
                        groupFlag
                        :value="curColumnConfig"
                        :data="columnConfig"
                        :default="defaultColumnConfig"
                        specialField="fixed"
                        specialText="固定左侧列"
                        saveTemplateFlag
                        templateRemindText="设置后将应用于所有报表"
                        :showFlag.sync="showFilterFlag"
                        @change="handleFilterChange">
      </customize-filter>
      
      <p class="demo-title">表格组件</p>
...
export default class demoExample extends Vue {
  ...
  /** pop-dialog */
  showFormDialogFlag: boolean = false;
  showInformDialogFlag: boolean = false;
  popFormDialog() {
