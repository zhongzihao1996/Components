<template>
  <el-select  size="mini"
              ...
              :disabled="disabled"
              :popper-class="`multi-select-popper ${smallSize? 'smallSize' : ''}`"
              multiple
              filterable
              reserve-keyword
    ...
    <div class="select-all">
      ...
      <button type="button"
              class="jh-button"
              @click="selectNone()">全不选</button>
    </div>

    <template v-if="group">
      <el-option-group
        v-for="(group, groupName, index1) in opts"
        :label="groupName"
        :key="index1"
...
.multi-select /deep/ .el-icon-arrow-up:before{content:"\e6e1"}
...
.el-select-group__wrap /deep/ .el-select-group__title {
  text-align: center;
  padding-left: 0;
  cursor: pointer;
  background: $stripeColor;
}
.multi-select /deep/ .el-select__tags > span {
  display: none;
}
.multi-select.disabled /deep/ .el-select__tags .el-select__input {
  cursor: not-allowed;
}
.multi-select /deep/ .el-input .count {
...
.multi-select /deep/ .el-input .el-input__prefix {
  width: 80%;
  white-space: nowrap;
  overflow: hidden;
...
.multi-select.disabled /deep/ .el-input i.el-input__icon {
  cursor: not-allowed;
}
.select-all {
  width: 100%;
